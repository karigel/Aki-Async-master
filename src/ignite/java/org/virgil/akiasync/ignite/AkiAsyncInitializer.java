...
public final class AkiAsyncInitializer {
    ...
    private IgniteConfigManager configManager;
    // 注意：cacheManager 在独立模式下暂不使用
    // private CacheManager cacheManager;
    private IgniteAkiAsyncBridge bridge;
    ...
    public static void initialize(final Logger logger) {
        ...
        synchronized (AkiAsyncInitializer.class) {
            ...
            // 创建 Ignite 专用的 Bridge 实现
            init.bridge = new IgniteAkiAsyncBridge(
                init.configManager,
            ...
            // 初始化异步路径查找
            if (init.configManager.isAsyncPathfindingEnabled()) {
                try {
                    org.virgil.akiasync.mixin.pathfinding.AsyncPathProcessor.initialize();
                    logger.info("[AkiAsync] Async pathfinding enabled with " + init.configManager.getAsyncPathfindingMaxThreads() + " threads");
                } catch (Exception e) {
                    logger.warning("[AkiAsync] Async pathfinding initialization failed: " + e.getMessage());
                }
            }
            ...
            logger.info("  [+] Block Entity Parallel: " + (init.configManager.isBlockEntityParallelTickEnabled() ? "Enabled" : "Disabled"));
            logger.info("  [+] Chunk Tick Async: " + (init.configManager.isChunkTickAsyncEnabled() ? "Enabled" : "Disabled"));
            logger.info("  [+] Falling Block Parallel: " + (init.configManager.isFallingBlockParallelEnabled() ? "Enabled" : "Disabled"));
            logger.info("  [+] Item Entity Parallel: " + (init.configManager.isItemEntityParallelEnabled() ? "Enabled" : "Disabled"));
            logger.info("  [+] Item Entity Smart Merge: " + (init.configManager.isItemEntityMergeOptimizationEnabled() ? "Enabled" : "Disabled"));
            logger.info("  [+] Universal AI: " + (init.configManager.isUniversalAiOptimizationEnabled() ? "Enabled" : "Disabled"));
            logger.info("  [+] BeeFix: " + (init.configManager.isBeeFixEnabled() ? "Enabled" : "Disabled"));
            logger.info("  [+] Minecart Cauldron: " + (init.configManager.isMinecartCauldronDestructionEnabled() ? "Enabled" : "Disabled"));
            logger.info("");
            ...
            logger.info("  [*] Network Optimization: " + (init.configManager.isNetworkOptimizationEnabled() ? "Enabled" : "Disabled"));
            logger.info("  [*] Fast Movement Chunk Load: " + (init.configManager.isFastMovementChunkLoadEnabled() ? "Enabled" : "Disabled"));
            logger.info("  [*] Center Offset Loading: " + (init.configManager.isCenterOffsetEnabled() ? "Enabled" : "Disabled"));
            logger.info("  [*] Structure Location Async: " + (init.configManager.isStructureLocationAsyncEnabled() ? "Enabled" : "Disabled"));
            logger.info("");
    ...
    private void loadConfiguration() {
        ...
        this.configManager = new IgniteConfigManager(dataFolder, logger);
        this.configManager.initialize();
        logger.info("[AkiAsync/Ignite] 配置文件已加载");
    }
    ...
    public IgniteAkiAsyncBridge getBridge() {
        return bridge;
    }
    ...
    public void reloadConfig() {
        ...
        // 异步路径查找处理器配置更新（如果启用）
        // AsyncPathProcessor config update - currently not supported in Ignite mode
        if (configManager.isAsyncPathfindingEnabled()) {
            logger.info("[AkiAsync] 异步路径查找已启用");
        }
