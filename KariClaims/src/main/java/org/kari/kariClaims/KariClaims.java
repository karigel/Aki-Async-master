    ...
    public void onEnable() {
        ...
        try {
            ...
            try {
                ...
                getLogger().info("SQLite 驱动已加载");
            } catch (ClassNotFoundException e) {
                getLogger().warning("无法预加载 SQLite 驱动: " + e.getMessage());
            }
            ...
            getServer().getPluginManager().registerEvents(new PowerCellListener(this), this);
            ...
            chunkClaimDAO.getAllPowerCellLocationsAsync()
                .thenAccept(locations -> {
                    ...
                    getLogger().info("已加载 " + count + " 个能源箱到缓存");
                })
                .exceptionally(throwable -> {
                    getLogger().warning("加载能源箱缓存失败: " + throwable.getMessage());
                    return null;
                });
