...
public class ChunkClaimManager {
    ...
    public void loadAllClaims() {
        ...
        chunkClaimDAO.getAllRegionsAsync().thenAccept(regions -> {
            ...
            plugin.getLogger().info("已加载 " + regions.size() + " 个领地区域");
            ...
            chunkClaimDAO.getAllChunkClaimsAsync().thenAccept(claims -> {
                ...
                plugin.getLogger().info("已加载 " + count + " 个区块领地");
                ...
                chunkClaimDAO.getAllClaimMembersAsync().thenAccept(members -> {
                    ...
                    plugin.getLogger().info("已加载 " + members.size() + " 个领地成员关系");
                }).exceptionally(e -> {
                    plugin.getLogger().warning("加载领地成员失败: " + e.getMessage());
                    return null;
                });
            }).exceptionally(e -> {
                plugin.getLogger().warning("加载区块领地失败: " + e.getMessage());
                return null;
            });
        }).exceptionally(e -> {
            plugin.getLogger().warning("加载领地区域失败: " + e.getMessage());
            return null;
        });
    }
    ...
    public CompletableFuture<Void> transferOwnership(int claimId, UUID newOwner) {
        return chunkClaimDAO.updateChunkClaimOwnerAsync(claimId, newOwner).thenRun(() -> {
            // 更新缓存
            invalidateCache();
        });
    }
    ...
    public CompletableFuture<Void> reloadCache() {
        ...
        return chunkClaimDAO.getAllRegionsAsync().thenCompose(regions -> {
            ...
            return chunkClaimDAO.getAllChunkClaimsAsync().thenAccept(claims -> {
                for (ChunkClaim claim : claims) {
                    String key = claim.getWorld() + ":" + claim.getChunkX() + ":" + claim.getChunkZ();
                    chunkCache.put(key, claim);
                    if (claim.getRegionId() > 0) {
                        ...
                        .thenAccept(claimOpt -> {
                            ...
                            plugin.getServer().getScheduler().runTask(plugin, () -> {
                                ...
                                if (chestBlock.getType() != org.bukkit.Material.CHEST) {
                                    plugin.getLogger().info("能量电池位置不是箱子，视为已破坏（领地 " + claimId + "）");
                                ...
                                plugin.getLogger().info("开始处理能量消耗（领地 " + claimId + "，需要消耗 " + seconds + " 秒）");
                                
                                // 计算当前箱子中的物品能源时间
                                long currentItemEnergy = calculateItemEnergy(chestInv, plugin);
                                long energyBeforeConsume = currentItemEnergy;
                                
                                // 扣除时间（优先物品，然后金钱，最后初始时间）
                                long remaining = seconds;
                                long requestedFromItems = 0;
                                
                                // 优先消耗物品时间
                                if (remaining > 0 && currentItemEnergy > 0) {
                                    requestedFromItems = Math.min(currentItemEnergy, remaining);
                                    
                                    plugin.getLogger().info("准备消耗 " + requestedFromItems + " 秒物品能源（领地 " + claim.getId() + "）");
                                    
                                    // 从箱子中消耗物品
                                    consumeEnergyFromChest(chestInv, requestedFromItems, plugin);
                                    
                                    // 重新计算箱子中的实际物品能源（验证消耗是否成功）
                                    long verifyEnergy = calculateItemEnergy(chestInv, plugin);
                                    plugin.getLogger().info("验证：消耗后箱子中剩余 " + verifyEnergy + " 秒物品能源（领地 " + claim.getId() + "）");
                                    
                                    // 使用实际计算的能源值，而不是理论值（因为可能有物品没有被正确消耗）
                                    claim.setEnergyTime(verifyEnergy);
                                    
                                    long effectiveConsumed = Math.max(0, energyBeforeConsume - verifyEnergy);
                                    if (effectiveConsumed == 0 && requestedFromItems > 0) {
                                        // 再尝试一次，以防状态未同步
                                        plugin.getLogger().warning("未能从箱子中扣除物品能源，重试一次（领地 " + claim.getId() + "）");
                                        consumeEnergyFromChest(chestInv, requestedFromItems, plugin);
                                        verifyEnergy = calculateItemEnergy(chestInv, plugin);
                                        claim.setEnergyTime(verifyEnergy);
                                        effectiveConsumed = Math.max(0, energyBeforeConsume - verifyEnergy);
                                    }
                                    
                                    long coveredByItems = Math.min(remaining, effectiveConsumed);
                                    remaining = Math.max(0, remaining - coveredByItems);
                                    
                                    plugin.getLogger().info("更新claim的energy_time为 " + verifyEnergy + " 秒（领地 " + claim.getId() + "，理论请求 " + requestedFromItems + " 秒，实际移除 " + effectiveConsumed + " 秒）");
                                } else {
                                    // 即使没有消耗物品时间，也要更新energy_time为箱子中的实际值
                                    claim.setEnergyTime(currentItemEnergy);
                                }
                                
                                // 然后消耗金钱时间
                                if (remaining > 0 && claim.getEconomyBalance() > 0) {
                                    double cost = remaining * pricePerSecond;
                                ...
                                // 最后消耗初始时间
                                if (remaining > 0 && claim.getInitialTime() > 0) {
                                    long consumedFromInitial = Math.min(claim.getInitialTime(), remaining);
     ...
     * 计算箱子中的物品能源时间
     */
    private long calculateItemEnergy(org.bukkit.inventory.Inventory chestInv, KariClaims plugin) {
        long totalEnergy = 0;
        ...
        for (int i = 0; i < chestInv.getSize(); i++) {
            ...
            for (String key : itemsSection.getKeys(false)) {
                ...
                if (itemType != null && itemType.equalsIgnoreCase(item.getType().name())) {
                    ...
                    totalEnergy += energyValue * item.getAmount();
                    break;
                }
            }
        }
    ...
    private void dissolveClaimAndDestroyPowerCell(ChunkClaim claim, Location powerCellLoc, KariClaims plugin) {
            ...
            if (region != null) {
                ...
                CompletableFuture.allOf(deletions.toArray(new CompletableFuture[0]))
                    ...
                    .thenRun(() -> {
                        ...
                        plugin.getLogger().info("区域 " + region.getId() + " (包含 " + region.getChunks().size() + " 个区块) 因能量耗尽已解散");
                    });
                return;
            }
        }
        ...
        plugin.getChunkClaimDAO().deleteChunkClaimAsync(claim.getId())
            .thenRun(() -> {
                ...
                plugin.getLogger().info("领地 " + claim.getId() + " 因能量耗尽已解散");
            });
    }
    ...
    private void dissolveClaimWithoutPowerCell(ChunkClaim claim, KariClaims plugin) {
        plugin.getChunkClaimDAO().deleteChunkClaimAsync(claim.getId())
            .thenRun(() -> {
                ...
                plugin.getLogger().info("领地 " + claim.getId() + " 因无能量电池且能量耗尽已解散");
            });
    }
    ...
    private long consumeEnergyFromChest(org.bukkit.inventory.Inventory chestInv, long energyToConsume, KariClaims plugin) {
        ...
        var itemsSection = plugin.getConfig().getConfigurationSection("power-cell.items");
        if (itemsSection == null) {
            plugin.getLogger().warning("power-cell.items 配置节不存在，无法消耗物品");
            return 0;
        }
        ...
        for (ItemEnergyEntry entry : itemsToConsume) {
            ...
            if (energyFromStack <= remainingEnergy) {
                // 消耗整个堆叠
                plugin.getLogger().info("消耗整个堆叠: " + item.getType() + " x" + item.getAmount() + " (价值: " + energyFromStack + " 秒)");
                ...
                plugin.getLogger().info("部分消耗: " + item.getType() + " x" + item.getAmount() + " -> x" + newAmount + " (消耗了 " + itemsNeeded + " 个，价值: " + energyConsumed + " 秒)");
        ...
        plugin.getLogger().info("物品消耗完成: 需要 " + energyToConsume + " 秒，实际消耗 " + totalConsumed + " 秒，剩余需求 " + remainingEnergy + " 秒");
