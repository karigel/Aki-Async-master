...
public class PowerCellGUI {
    ...
    public void openPowerCellGUI(Player player, ChunkClaim claim, PowerCell powerCell) {
        ...
        Inventory gui = Bukkit.createInventory(null, 54, LegacyComponentSerializer.legacySection().deserialize(ownerName + "的领地"));
    ...
    private void setupFirstRow(Inventory gui, ChunkClaim claim, PowerCell powerCell) {
        ...
        long totalTime = powerCell.getTotalRemainingTime();
        String timeStr = formatTime(totalTime);
        clockLore.add(LegacyComponentSerializer.legacySection().deserialize("§6总能量: §e" + timeStr));
        clockLore.add(LegacyComponentSerializer.legacySection().deserialize("§7物品燃料: §a" + formatTime(powerCell.getEnergyTime())));
        ...
        itemTimeLore.add(LegacyComponentSerializer.legacySection().deserialize("§7物品燃料剩余: §a" + formatTime(powerCell.getEnergyTime())));
    ...
    public void openSettingsGUI(Player player, ChunkClaim claim) {
        ...
        // 飞行
        gui.setItem(14, createToggleItem(Material.FEATHER, "飞行", claim.isFly(), "fly"));
        // 锁定
        gui.setItem(15, createToggleItem(Material.IRON_DOOR, "领地锁定", claim.isLocked(), "locked"));
