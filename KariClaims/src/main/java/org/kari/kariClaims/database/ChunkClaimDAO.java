...
public class ChunkClaimDAO {
    ...
    private org.kari.kariClaims.KariClaims plugin;

    public ChunkClaimDAO(DatabaseManager databaseManager) {
        this.databaseManager = databaseManager;
    }
    
    public void setPlugin(org.kari.kariClaims.KariClaims plugin) {
        this.plugin = plugin;
    }
    ...
    public void createTables() throws SQLException {
        ...
        String createChunkClaimsTable = isMySQLOrMariaDB()
            ? "CREATE TABLE IF NOT EXISTS chunk_claims (" +
                ...
                "home_public BOOLEAN DEFAULT FALSE, " +
    ...
    public CompletableFuture<ChunkClaim> createChunkClaimAsync(UUID owner, Chunk chunk) {
        return CompletableFuture.supplyAsync(() -> {
            String sql = "INSERT INTO chunk_claims (owner, world, chunk_x, chunk_z, claimed_at) VALUES (?, ?, ?, ?, ?)";
    ...
    public CompletableFuture<Void> updateChunkClaimAsync(ChunkClaim claim) {
        return CompletableFuture.runAsync(() -> {
            String sql = "UPDATE chunk_claims SET chunk_name = ?, home_x = ?, home_y = ?, home_z = ?, " +
                ...
                "entity_drop = ?, water_flow = ?, fly = ?, mob_griefing = ?, tnt = ?, region_id = ?, " +
                "visitor_permissions = ?, member_permissions = ? WHERE id = ?";
            ...
            try (Connection conn = databaseManager.getConnection();
                ...
                stmt.setBoolean(20, claim.isFly());
                stmt.setBoolean(21, claim.isMobGriefing());
                stmt.setBoolean(22, claim.isTnt());
                stmt.setInt(23, claim.getRegionId());
                stmt.setInt(24, claim.getVisitorPermissions());
                stmt.setInt(25, claim.getMemberPermissions());
    ...
    */
    ...
    private ChunkClaim mapResultSetToChunkClaim(ResultSet rs) throws SQLException {
        ...
        try {
            ...
            claim.setFly(rs.getBoolean("fly"));
            ...
            try (PreparedStatement checkStmt = conn.prepareStatement(checkColumnSql);
                ...
                if (rs.next() && rs.getInt(1) == 0) {
                    ...
                    try (Statement alterStmt = conn.createStatement()) {
                        ...
                        plugin.getLogger().info("已添加initial_time列到chunk_claims表");
                    }
                }
            }
        } catch (SQLException e) {
            plugin.getLogger().warning("检查/添加initial_time列时出错: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            try (PreparedStatement checkStmt = conn.prepareStatement(checkColumnSql);
                ...
                if (rs.next() && rs.getInt(1) == 0) {
                    ...
                    try (Statement alterStmt = conn.createStatement()) {
                        alterStmt.execute(alterSql);
                        plugin.getLogger().info("已添加external_fluid_inflow列到chunk_claims表");
                    }
                }
            }
        } catch (SQLException e) {
            plugin.getLogger().warning("检查/添加external_fluid_inflow列时出错: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            try (PreparedStatement checkStmt = conn.prepareStatement(checkColumnSql);
                ...
                if (rs.next() && rs.getInt(1) == 0) {
                    ...
                    try (Statement alterStmt = conn.createStatement()) {
                        alterStmt.execute(alterSql);
                        plugin.getLogger().info("已添加region_id列到chunk_claims表");
                    }
                }
            }
        } catch (SQLException e) {
            plugin.getLogger().warning("检查/添加region_id列时出错: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            try (PreparedStatement checkStmt = conn.prepareStatement(checkVisitorSql);
                ...
                if (rs.next() && rs.getInt(1) == 0) {
                    ...
                    try (Statement alterStmt = conn.createStatement()) {
                        alterStmt.execute(alterSql);
                        plugin.getLogger().info("已添加visitor_permissions列到chunk_claims表");
                    }
                }
            }
            ...
            try (PreparedStatement checkStmt = conn.prepareStatement(checkMemberSql);
                ...
                if (rs.next() && rs.getInt(1) == 0) {
                    ...
                    try (Statement alterStmt = conn.createStatement()) {
                        alterStmt.execute(alterSql);
                        plugin.getLogger().info("已添加member_permissions列到chunk_claims表");
                    }
                }
            }
        } catch (SQLException e) {
            plugin.getLogger().warning("检查/添加权限列时出错: " + e.getMessage());
        }
    }
    ...
    public CompletableFuture<org.bukkit.Location> getPowerCellLocationAsync(int claimId) {
        return CompletableFuture.supplyAsync(() -> {
            String sql = "SELECT * FROM power_cells WHERE claim_id = ? LIMIT 1";
    ...
    public CompletableFuture<Void> deletePowerCellAsync(int claimId) {
        return CompletableFuture.runAsync(() -> {
            String sql = "DELETE FROM power_cells WHERE claim_id = ?";
